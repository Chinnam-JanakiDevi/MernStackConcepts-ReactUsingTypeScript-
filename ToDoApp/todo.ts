        ...
        this.todoTasks.forEach((todoTask: TodoTask) => {
            this.todoItemsHTML.innerHTML += `
                <li class="${todoTask.completed ? 'completed' : 'notcompleted'}">
                    ${todoTask.task}
                    <button onclick="todoApp.markCompleted(${todoTask.id})"><i class="bi bi-check"></i></button>
                    <button onclick="todoApp.removeTask(${todoTask.id})"><i class="bi bi-x-lg"></i></button>
                </li>
            `
        })
    }
